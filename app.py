...
def authorized():
    ...
    try:
        ...
        print(f"User  info stored in session: {session['user']}")  # Debugging
        ...
        if not user:
            ...
            db.session.commit()
            print(f"New user created: {new _user}")  # Debugging
            user = new_user

        # Store user role in session
        session['user_role'] = user.role
        print(f"User  role stored in session: {session['user_role']}")  # Debugging
...
def _build_auth_url(scopes=None, state=None):
    return f"{AUTHORITY}/oauth2/v2.0/authorize?client_id={CLIENT_ID}&response_type=code&redirect_uri={REDIRECT_URI}&response_mode=query&scope={' '.join(scopes)}&state={state}"
...
def _get_user_info(token):
    try:
        ...
        graph_data.raise_for_status()  # Raise an error for bad responses
        return graph_data.json()  # Return the user info as JSON
    except requests.exceptions.HTTPError as err:
        logger.error(f"HTTP error occurred: {err}")  # Logging
        return None
    except Exception as e:
        logger.error(f"Error fetching user info: {e}")  # Logging
        return None
